...
from thunderbird.github import GithubFile, GithubDownloader
...
class TestGithubDownloader(Basetest):
    ...
    def test_github_files(self):
        ...
        for url, expected_repo, expected_file_path in self.test_urls:
            ...
            self.assertEqual(gh_file.repo, expected_repo, f"Repo mismatch for URL: {url}")
            self.assertEqual(gh_file.file_path, expected_file_path, f"File path mismatch for URL: {url}")
